...
public class PostServiceImpl implements PostService {
	...
	public void showPostList() {
		...
		for (int i = 0; i < posts.size(); i++) {
			...
			System.out.println(i + "." + post.getTitle() + "댓글 :" + post.getComments().size() + "개" + "좋아요 :"
					+ post.getLikeUsers().size() + "개" + "조회수 :" + post.getViewedUsers().size() + "개");
		}
	}
	...
	public void showPostDetail() {
		...
		for (Comment comment : post.getComments()) {
			System.out.println(comment.getAuthor().getName() + ": " + comment.getContent() + " (작성 날짜: "
					+ comment.getAddTime().format(format) + ", 좋아요 수: " + comment.getLikeUsers().size() + ")");
		}
